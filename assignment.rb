...
class Solution
  ...
  def age_groups_faster_than age_group, criteria_time
    @coll.find.aggregate([ {:$match=>{:age=>"$#{criteria_time}"}}, 
                           {:$group=>{:_id=>{:gender=>"$gender"}, runners:{:$sum=>1}, fastest_time:{:$min=>"$secs"}}}, 
                           {:$match=>{:fastest_time=>{:$lte=>criteria_time}}}
                           ])
  end
  ...
  def number_goal last_name
    # accepts a last_name
    # finds the racers having that same last name
    # determines the average of all their race times
    # forms an array of numbers for each member of the group

    # forms a result with avg_time for each number
    # forms a result with last_name, number, and avg_time for each number in the family with no _id property 
